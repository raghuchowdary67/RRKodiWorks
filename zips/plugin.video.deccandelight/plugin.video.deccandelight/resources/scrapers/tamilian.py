...
import re
import requests
import urlresolver.lib.urlresolver as resolveurl


class tamilian(Scraper):
    def __init__(self):
        Scraper.__init__(self)
        self.bu = 'https://tamilian.net/'
        self.icon = self.ipath + 'tamilian.png'

    def get_menu(self):
        html = requests.get(self.bu, headers=self.hdr).text
        items = {'01Latest': self.bu + 'hd-movies-online/'}
    ...
    def get_items(self, url):
        ...
        for item in items:
            title = h.unescape(item.find("span", {"class": "movie-title"}).text.strip())
    ...
    def get_video(self, url):
        ...
        eurl = re.findall(r'<iframe.+?src="([^"]+)', shtml, re.I)[0].split('?')[0]
