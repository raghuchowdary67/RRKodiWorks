...
class skytamil(Scraper):
    ...
    def get_url(self, url, headers=None):
        ...
        if resp.status_code == 403 and "DDoS protection by DDoS-GUARD" in resp.text:
            su = urllib_parse.urlparse(url)
            host = '{0}://{1}/'.format(su.scheme, su.netloc)
    ...
    def get_menu(self):
        ...
        items = {'01Sun TV Serials': self.bu + 'category/sun-tv-serials/',
                 '02Sun TV Shows': self.bu + 'category/tamil-tv-shows/sun-tv-shows/'}
    ...
    def get_items(self, url):
        ...
        for item in items:
            title = h.unescape(item.find('h4').text)
    ...
    def get_video(self, url):
        ...
        headers.update({'Referer': self.bu})
        html = self.get_url(url, headers=headers)
        items = re.findall(r'''<iframe\s*src=["']([^"']+)''', html)

        for item in items:
            sources.append(self.get_vidhost(item))
            videos.append(item)
        ...
        ehtml = self.get_url(eurl, headers=headers)
        if jsunpack.detect(ehtml):
            ehtml = jsunpack.unpack(ehtml).replace('\\', '')

        r = re.search(r'''file['"]?\s*:\s*["']([^"']+)''', ehtml)
        if r:
            surl = r.group(1)
            if surl.startswith('/'):
                surl = urllib_parse.urljoin(eurl, surl)
            return surl
