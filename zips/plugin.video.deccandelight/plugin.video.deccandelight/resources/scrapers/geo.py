...
class geo(Scraper):
    ...
    def get_items(self, url):
        ...
        if catid:
            ...
            while nextpg:
                ...
                for item in items:
                    title = h.unescape(item.find_all('p')[-1].text)
                    title = title.encode('utf8') if self.PY2 else title
    ...
    def get_video(self, url):
        ...
        js = requests.get(jsurl.format(acct, player), headers=headers).text
        pk = re.findall('policyKey:"([^"]+)', js)[0]
        headers.update({'Accept': 'application/json;pk={0}'.format(pk)})
        sources = requests.get(apiurl.format(acct, vid), headers=headers).json()['sources']
